...
package ys.components.style {
	...
	public class StyleManager
	{
		...
		public function StyleManager() {
			...
			globalStyles = YComponent.getStyleDefinition();
		}
		 ...
		 */
		public static function registerInstance(instance:YComponent):void {
				...
				while (defaultStyles == null) {
					...
					try {
						...
						try {
							...
							defaultStyles = YComponent.getStyleDefinition();
							break;
						}
					}
				}
		...
		public static function unregisterInstance(instance:YComponent):void
		{
			var inst:StyleManager = getInstance();
			var classDef:Class = getClassDef(instance);
			...
			delete inst.classToInstancesDict[classDef][instance];
		}
		
		/**
		 * Sets an inherited style on a component.
		 */
		private static function setSharedStyles(instance:YComponent):void {
			var inst:StyleManager = getInstance();
			var classDef:Class = getClassDef(instance);
...
//				instance.setSharedStyle(n,getSharedStyle(instance,n));
				//使用此方式时,后面设置的style将覆盖共享的style,即使用其值为null;
				//所有的共享style需要开始时设置,否则会覆盖自定义的style
				instance.setStyle(n,getSharedStyle(instance,n));
			}
		}
		
		private static function getSharedStyle(instance:YComponent,name:String):Object {
			var classDef:Class = getClassDef(instance);
			var inst:StyleManager = getInstance();
		...
		private static function invalidateComponentStyle(componentClass:Class,name:String):void {
			...
			for (var obj:Object in instances) {
				var instance:YComponent = obj as YComponent;
				if (instance == null) { continue; }
				//原fl组件采用的方式
//				instance.setSharedStyle(name,getSharedStyle(instance,name));
				//使用此方式时,后面设置的style将覆盖共享的style,即使用其值为null;
				//所有的共享style需要开始时设置,否则会覆盖自定义的style
				instance.setStyle(name,getSharedStyle(instance,name));
			}
		}
