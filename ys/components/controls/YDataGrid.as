package ys.components.controls {
	...
	import ys.components.controls.YSelectableList;
	import ys.components.controls.YTextInput;
	import ys.components.controls.dataGridClasses.DataGridColumn;
	import ys.components.controls.dataGridClasses.HeaderRenderer;
	...
	import ys.components.events.DataGridEvent;
	import ys.components.events.DataGridEventReason;
	 ...
	 */
	[Event(name="headerRelease", type="ys.components.events.DataGridEvent")]

	/**
	 * Dispatched after a user expands a column horizontally.
	 */
	[Event(name="columnStretch", type="ys.components.events.DataGridEvent")]

	/**
	 * Dispatched after a user prepares to edit an item, for example,
	 * by releasing the mouse button over the item.
	 */
	[Event(name="itemEditBeginning", type="ys.components.events.DataGridEvent")]

	/**
	 *  Dispatched after the <code>editedItemPosition</code> property is set
	 *  and the item can be edited.
	 */
	[Event(name="itemEditBegin", type="ys.components.events.DataGridEvent")]

	/**
	 *  Dispatched when an item editing session ends for any reason.
	 */
	[Event(name="itemEditEnd", type="ys.components.events.DataGridEvent")]

	/**
	 *  Dispatched after an item receives focus.
	 */
	[Event(name="itemFocusIn", type="ys.components.events.DataGridEvent")]

	/**
	 *  Dispatched after an item loses focus.
	 */
	[Event(name="itemFocusOut", type="ys.components.events.DataGridEvent")]

	public class YDataGrid extends YSelectableList implements IFocusManagerComponent 
	{
		...
		public static function getStyleDefinition():Object {
			return mergeStyles(defaultStyles, YSelectableList.getStyleDefinition(), YScrollBar.getStyleDefinition());
		}

		public function YDataGrid() {
			super();
			if (_columns == null) {
				_columns = []; 
			}
			_horizontalScrollPolicy = YScrollPolicy.OFF;
			activeCellRenderersMap = new Dictionary(true);
			availableCellRenderersMap = new Dictionary(true);
			addEventListener(DataGridEvent.ITEM_EDIT_BEGINNING, itemEditorItemEditBeginningHandler, false, -50, true);
			addEventListener(DataGridEvent.ITEM_EDIT_BEGIN, itemEditorItemEditBeginHandler, false, -50, true);
			addEventListener(DataGridEvent.ITEM_EDIT_END, itemEditorItemEditEndHandler, false, -50, true);
		...
		public function set rowCount(value:uint):void {
			...
			var scrollBarHeight:Number = (_horizontalScrollPolicy == YScrollPolicy.ON || (_horizontalScrollPolicy == YScrollPolicy.AUTO && _hScrollBar)) ? 15 : 0;
...
//						copyStylesToChild(headerRenderer, HEADER_STYLES);
				...
				if(length > 0) {
					for (i = rowStartIndex; i <= rowEndIndex; i++) {
						...
						if (renderer is YComponent) {
							rendererUIC = renderer as YComponent;
							rendererUIC.drawNow();
						}
					}
				}
		...
		protected function calculateColumnSizes():void {
			...
			if (columnsInvalid) {
				...
				if (horizontalScrollPolicy == YScrollPolicy.OFF) {
		...
		public function resizeColumn(columnIndex:int, w:Number):void {
			...
			if (_horizontalScrollPolicy == YScrollPolicy.ON || _horizontalScrollPolicy == YScrollPolicy.AUTO) {
		...
		override protected function focusInHandler(event:FocusEvent):void {
			...
			if (editable) {
				...
				addEventListener(MouseEvent.MOUSE_DOWN, mouseFocusChangeHandler, false, 0, true);
			}
		}
		...
		public function createItemEditor(colIndex:uint, rowIndex:uint):void {
			...
			DisplayObject(itemEditorInstance).addEventListener(FocusEvent.FOCUS_OUT, itemEditorFocusOutHandler, false, 0, true);
			rendererSprite.visible = false;
			// listen for keyStrokes on the itemEditorInstance (which lets the grid supervise for ESC/ENTER)
			DisplayObject(itemEditorInstance).addEventListener(KeyboardEvent.KEY_DOWN, editorKeyDownHandler, false, 0, true);
		...
		public function destroyItemEditor():void {
			if (itemEditorInstance) {
				...
				if (itemEditorInstance && itemEditorInstance is YComponent) {
					YComponent(itemEditorInstance).drawFocus(false);
				}
